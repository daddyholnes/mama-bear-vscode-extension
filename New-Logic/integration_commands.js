...
const mama_bear_agent_model_orchestrator_1 = require("./mama_bear_agent_model_orchestrator");
// Add to your activate function:
function activate(context) {
    // ... existing code ...
    // Initialize Agent-Model Orchestrator
    const agentOrchestrator = new mama_bear_agent_model_orchestrator_1.MamaBearAgentModelOrchestrator(mamaBearApiClient);
    ...
    const orchestratorCommands = [
        ...
        vscode.commands.registerCommand('mamaBear.deepThinkerAnalyze', async () => {
            ...
            const result = await agentOrchestrator.routeWorkflow({
                task: mama_bear_agent_model_orchestrator_1.WorkflowTask.OPTIMIZATION,
                context: {
                    code: selectedText,
        ...
        vscode.commands.registerCommand('mamaBear.codeSurgeonFix', async () => {
            ...
            const result = await agentOrchestrator.rapidDebugging('Fix and optimize this code', selectedText);
        ...
        vscode.commands.registerCommand('mamaBear.creativeGeniusDesign', async () => {
            ...
            if (designType) {
                ...
                if (requirements) {
                    const result = await agentOrchestrator.routeWorkflow({
                        task: mama_bear_agent_model_orchestrator_1.WorkflowTask.DOCUMENTATION,
        ...
        vscode.commands.registerCommand('mamaBear.integrationMasterConnect', async () => {
            ...
            if (integrationType) {
                ...
                if (specs) {
                    const result = await agentOrchestrator.routeWorkflow({
                        task: mama_bear_agent_model_orchestrator_1.WorkflowTask.INTEGRATION,
    ...
    // Helper functions
    async function showAgentResult(result, title) {
    ...
    async function insertCodeAtCursor(editor, content) {
        const selection = editor.selection;
        await editor.edit(editBuilder => {
            editBuilder.replace(selection, content);
        });
    }
    async function showCodeFixSuggestion(editor, result) {
        const action = await vscode.window.showInformationMessage(`⚕️ Code Surgeon found issues and has fixes ready!`, 'Apply Fix', 'View Analysis', 'Cancel');
        switch (action) {
            case 'Apply Fix':
    ...
    async function showAgentDashboard(stats, activeWorkflows) {
...
${agentOrchestrator.getWorkflowHistory(10).map((w, i) => `
...
`;
    ...
    async function determineTaskType(description) {
        // Simple AI-based task classification
        const lowerDesc = description.toLowerCase();
        if (lowerDesc.includes('plan') || lowerDesc.includes('architect'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.PROJECT_PLANNING;
        if (lowerDesc.includes('setup') || lowerDesc.includes('environment'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.ENVIRONMENT_SETUP;
        if (lowerDesc.includes('generate') || lowerDesc.includes('create') || lowerDesc.includes('build'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.CODE_GENERATION;
        if (lowerDesc.includes('review') || lowerDesc.includes('check'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.CODE_REVIEW;
        if (lowerDesc.includes('debug') || lowerDesc.includes('fix') || lowerDesc.includes('error'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.DEBUGGING;
        if (lowerDesc.includes('test'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.TESTING;
        if (lowerDesc.includes('optimize') || lowerDesc.includes('performance'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.OPTIMIZATION;
        if (lowerDesc.includes('deploy') || lowerDesc.includes('production'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.DEPLOYMENT;
        if (lowerDesc.includes('document') || lowerDesc.includes('readme'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.DOCUMENTATION;
        if (lowerDesc.includes('integrate') || lowerDesc.includes('api') || lowerDesc.includes('connect'))
            return mama_bear_agent_model_orchestrator_1.WorkflowTask.INTEGRATION;
        return mama_bear_agent_model_orchestrator_1.WorkflowTask.CODE_GENERATION; // Default
    }
    async function assessComplexity(description) {
    ...
    async function showSmartRouteResult(result, originalRequest) {
...
//# sourceMappingURL=integration_commands.js.map