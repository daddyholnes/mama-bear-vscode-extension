...
export class MamaBearGitProvider {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const repoInfo = {
                    ...
                    indexChanges: repo.state.indexChanges.length,                        remotes: repo.state.remotes.map((remote: any) => ({
                            name: remote.name,
                            url: remote.fetchUrl
                        })),
...
Format in markdown for GitHub/GitLab.`;
            ...
            if (response.success && response.response) {
                ...
                vscode.window.showInformationMessage(
                    '📝 Pull request description generated! Copy to your Git hosting service.',
                    'Open GitHub', 'Open GitLab'
                ).then(selection => {
                    if (selection) {
    ...
    public async suggestBranchName(description: string): Promise<string | undefined> {
        try {
...
Return ONLY the branch name.`;
    ...
    private async getCommitCount(repo: any): Promise<number> {
        try {
    ...
    private async getContributors(repo: any): Promise<string[]> {
        try {
    ...
    private async getRecentCommits(repo: any, count: number): Promise<any[]> {
        try {
    ...
    private async analyzeChanges(repo: any): Promise<any> {
        const stagedChanges = repo.state.indexChanges;
        const workingTreeChanges = repo.state.workingTreeChanges;
    ...
    private summarizeChanges(stagedChanges: any[], workingTreeChanges: any[]): string {
    ...
    private createChangesSummary(changeTypes: any, stagedChanges: any[], workingTreeChanges: any[]): string {
        const parts = [];
        ...
        if (changeTypes.deleted > 0) parts.push(`${changeTypes.deleted} deleted`);
        if (changeTypes.renamed > 0) parts.push(`${changeTypes.renamed} renamed`);
    ...
    private extractBranchName(response: string): string {
    ...
    private async showRepositoryAnalysis(data: any): Promise<void> {
    ...
    private formatRepositoryReport(data: any): string {
...
${repoInfo.remotes.map((remote: any) =>
...
*Generated by Mama Bear AI Assistant on ${new Date().toLocaleString()}*`;
    ...
    private openGitHostingService(repo: any, service: string): void {
